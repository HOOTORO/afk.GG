...
const relics = JSON.parse(`
  ...
  `);
const relicLeveling = JSON.parse(`
  {
  "might": {
    "10": [1, 2, 5, 6, 4, 3],
...
export { AbexHelper, Beasts, Heroes, based, classes, races, relicLeveling, relicType, relics, townTypes, };
class AbexHelper {
    static tier(id) {
        const tier = relics.find((x) => x.id === id)?.tier;
        return tier;
    }
    static defaultRelics(branch) {
        Object.entries(relicLeveling[branch]).map((x) => { });
        return relicLeveling[branch][10].map((x) => x);
    }
    static lowerTierRelic(branch, id) {
        const current = this.tier(id), position = relicLeveling[branch][current].findIndex((x) => x === id);
        return relicLeveling[branch][current - 10][position];
    }
    static relicPrice(id) {
    ...
    static nextTier(current, goal = false) {
        if (goal) {
            switch (current) {
                case 50:
                    return;
