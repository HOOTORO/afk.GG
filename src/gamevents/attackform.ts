...
tree.forEach((x) => {
  treeDiv.appendChild(
    createInput("number", "", `/afk.GG/assets/icons/tree/tree-${x}.png`, {
      class: `${formId}-inputs-number`,
...
fields.forEach((v) => {
  ...
  heroForm.addEventListener("change", (x: InputEvent) => {
    const tg = x.target as HTMLSelectElement;
    const op = tg.value;
    tg.querySelector("option[selected]").removeAttribute("selected");
    tg.querySelector(`option[value*="${op}"]`).setAttribute("selected", "");
...
).insertAfter(`.${formId} input, #${formId} input`);
...
export function addMemember() {
  // const heroname = document.getElementById("afk-heroes") as HTMLSelectElement;
  const current = getCurrentHero();
  if (
    MetaTeam.Heroes().findIndex((x) => x.name === current.name) === -1 &&
    MetaTeam._len < 5
  ) {
...
function heroF(h: Hero, p: number) {
...
function getSelectedById(id: string) {
